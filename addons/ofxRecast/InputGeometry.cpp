...
bool InputGeometry::loadMesh(const char* filepath)
{
	if (m_mesh)
	{
		delete m_chunkyMesh;
...
bool InputGeometry::load(const char* filePath)
{
	struct local
	{
...
bool InputGeometry::save(const char* filepath)
{
...
bool InputGeometry::raycastMesh(float* src, float* dst, float& tmin)
{
	float dir[3];
	rcVsub(dir, dst, src);
...
void InputGeometry::addConvexVolume(const float* verts, const int nverts,
...
void InputGeometry::deleteConvexVolume(int i)
{
	m_volumeCount--;
