...
void ofxSpriteQuad::UpdateVisibility(bool force_update, bool camera_move)
{
	...
	if(!camera_move && 
		(m_Status == QUAD_STATUS_NO_CHANGE || m_Status == QUAD_STATUS_MATERIAL_CHANGE))
		return;
	if(!force_update && 
	...
	{
		if(m_Status != QUAD_STATUS_POSITION_CHANGE && m_Status != QUAD_STATUS_MATERIAL_CHANGE)
			m_Status = QUAD_STATUS_VISIBILITY_CHANGE;
	}
}
